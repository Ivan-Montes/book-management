...
@Component
public class KafkaAuthorSubscriberAdapter implements SubscriberPort {

	private final ProjectorPort projectorPort;
	private final Map<String, Consumer<Event>> actionsMap;
	private static final Logger logger = LoggerFactory.getLogger(KafkaAuthorSubscriberAdapter.class);
	private final KafkaSubscriberValidationHelper kafkaSubscriberValidationHelper;

	public KafkaAuthorSubscriberAdapter(@Qualifier("authorProjectorAdapter")ProjectorPort projectorPort,
	...
	private Map<String, Consumer<Event>> initializeActionsMap() {
		...
		return Map.of(GlobalConstants.AUTHOR_CREATED, projectorPort::create, GlobalConstants.AUTHOR_UPDATED,
				projectorPort::update, GlobalConstants.AUTHOR_DELETED, projectorPort::deleteById);
	}

	@KafkaListener(topics = { GlobalConstants.AUTHOR_CREATED, GlobalConstants.AUTHOR_UPDATED, GlobalConstants.AUTHOR_DELETED },
			groupId = "bookmgmt-consumer-author")
	@Override
