...
public class BookshopReadRepositoryAdapter implements ReadRepositoryPort<Bookshop>, RequestByNameReadRepositoryPort {
	...
	public BookshopReadRepositoryAdapter(SessionFactory sessionFactory, BookshopMapper mapper) {
		super();
	...
	@Override
	public Mono<Page<Bookshop>> findAll(Pageable pageable) {

		CriteriaBuilder cb = sessionFactory.getCriteriaBuilder();
		CriteriaQuery<BookshopJpaEntity> cq = cb.createQuery(BookshopJpaEntity.class);
		Root<BookshopJpaEntity> root = cq.from(BookshopJpaEntity.class);
	    ...
	    Root<BookshopJpaEntity> countRoot = countQuery.from(BookshopJpaEntity.class);
	    ...
	    Mono<List<BookshopJpaEntity>> entityMono = Mono.from(dataQuery.convert().with(UniReactorConverters.toMono()));
	    Mono<Long> countMono = Mono.from(countUni.convert().with(UniReactorConverters.toMono()));

	    return Mono.zip(entityMono, countMono)
	        .map(tuple -> {
	            List<Bookshop> domList = convertToDom(tuple.getT1());
	            long total = tuple.getT2();
	            return convertToPage(domList, pageable, total);
	        });
	}

	private List<Bookshop> convertToDom(List<BookshopJpaEntity> list) {
	...
	private Page<Bookshop> convertToPage(List<Bookshop> list, Pageable pageable, long total) {
		
		return new PageImpl<>(list, pageable, total);
	}
	
	private List<Order> buildSortOrder(Sort sort, CriteriaBuilder criteriaBuilder, Root<BookshopJpaEntity> root) {
	...
	@Override
	public Mono<Bookshop> findById(UUID id) {
		...
		String queryString = """
				...
				FROM
				 BookshopJpaEntity e
				LEFT JOIN FETCH e.books
				WHERE e.bookshopId = ?1
				""";

		return sessionFactory
				.withSession(session -> session.createQuery(queryString, BookshopJpaEntity.class)
						.setParameter(1, id)
	...
	public Mono<Boolean> existsByName(String name) {
		...
		String jpql = "SELECT COUNT(b) FROM BookshopJpaEntity b WHERE b.name = :name";
		return sessionFactory
				.withSession(
	...
	public Mono<Boolean> findByNameAndIdNot(String name, UUID id) {
	    ...
	    Predicate conditionId = cb.notEqual(root.get(GlobalConstants.BS_ID), id);
