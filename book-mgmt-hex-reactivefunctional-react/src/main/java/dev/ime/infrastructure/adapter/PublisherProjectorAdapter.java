...
public class PublisherProjectorAdapter implements ProjectorPort {
	...
	private Mono<PublisherJpaEntity> validateCreate(PublisherJpaEntity entity) {
	...
	private Mono<PublisherJpaEntity> checkNameAvailability(PublisherJpaEntity entity) {
		...
		return sessionFactory
				...
				.switchIfEmpty(Mono.error(new UniqueValueException(
						Map.of(GlobalConstants.PUBLI_NAME, entity.getName()))))
				.thenReturn(entity);
	}
	...
	private Mono<PublisherJpaEntity> validateUpdate(PublisherJpaEntity entity) {
	...
	private Mono<PublisherJpaEntity> checkNameAvailabilityInUpdateAction(PublisherJpaEntity entity) {
		...
		return sessionFactory
				.withSession(session -> session.createQuery(query, Long.class)
						...
						.setParameter(GlobalConstants.PUBLI_ID, entity.getPublisherId()).getSingleResult()
						.map(count -> count == 0))
				.convert().with(UniReactorConverters.toMono()).filter(b -> b)
				.switchIfEmpty(Mono.error(new UniqueValueException(
						Map.of(GlobalConstants.PUBLI_NAME, entity.getName()))))
	...
	private Mono<UUID> checkEntityAssociation(UUID entityId){
		...
		return sessionFactory
				.withSession(session -> session.createNativeQuery(query, Long.class)
						...
						.convert().with(UniReactorConverters.toMono())
						.filter( i -> i == 0)
						.switchIfEmpty(Mono.error(new EntityAssociatedException(Map.of(GlobalConstants.PUBLI_ID, entityId.toString()))))
						.thenReturn(entityId);
	}
