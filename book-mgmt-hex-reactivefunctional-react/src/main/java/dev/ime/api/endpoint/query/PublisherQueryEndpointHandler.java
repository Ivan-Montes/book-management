...
@Component
@Qualifier("publisherQueryEndpointHandler")
public class PublisherQueryEndpointHandler implements QueryEndpointPort {

	private static final Logger logger = LoggerFactory.getLogger(PublisherQueryEndpointHandler.class);
	private final QueryServicePort<PublisherDto> publisherService;
    private final PaginationUtils paginationUtils;
	private final DtoValidator dtoValidator;
	private final ApiExceptionHandler apiExceptionHandler;
	
	public PublisherQueryEndpointHandler(QueryServicePort<PublisherDto> publisherService, PaginationUtils paginationUtils,
	...
	@Operation(
            ...
            description = "Returns a list of all available publishers"
        )
    @ApiResponse(
        responseCode = "200", 
        description = "List of publishers found",
        content = @Content(
	...
	public Mono<ServerResponse> getAll(ServerRequest serverRequest) {
		...
		return paginationUtils.createPaginationDto(serverRequest, PublisherDto.class)
    ...
    @Operation(
            ...
            description = "Returns a specific publishers based on its ID",
            parameters = {
                    @Parameter(name = GlobalConstants.MODEL_ID, in = ParameterIn.PATH, required = true, description = "publisher ID", schema = @Schema(type = "string", format = "uuid"))
                }
    		)
    @ApiResponse(
        responseCode = "200", 
        description = "publishers found",
        content = @Content(
            mediaType = "application/json",
            schema = @Schema(implementation = PublisherDto.class)
        )
	...
	public Mono<ServerResponse> getById(ServerRequest serverRequest) {
    	...
    	logger.info("GET /publishers/{} - Retrieve by ID", GlobalConstants.MODEL_ID);
		...
		return Mono.defer( () -> {
			...
			try {
	            ...
	            return apiExceptionHandler.handleException(new InvalidUUIDException(Map.of(GlobalConstants.PUBLI_ID, GlobalConstants.MSG_NODATA)));	            
	        }			
		});
	}
}
