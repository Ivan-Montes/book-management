...
@Configuration
public class AuthorQueryEndpointRouter {

	private final UriConfigProperties uriConfigProperties;
	private final QueryEndpointPort queryEndpointPort;

	public AuthorQueryEndpointRouter(UriConfigProperties uriConfigProperties,
			@Qualifier("authorQueryEndpointHandler")QueryEndpointPort queryEndpointPort) {
		super();
	...
	@RouterOperations({
		@RouterOperation(
				path = "/api/v1/authors", 
				beanClass = AuthorQueryEndpointHandler.class, 
				beanMethod = "getAll",
	...
	@Bean
	RouterFunction<ServerResponse> authorQueryEndpointRoutes(){
