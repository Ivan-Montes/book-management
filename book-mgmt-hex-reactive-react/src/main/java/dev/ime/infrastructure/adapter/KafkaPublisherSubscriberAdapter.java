...
@Component
public class KafkaPublisherSubscriberAdapter implements SubscriberPort {

	private final ProjectorPort projectorPort;
	private final Map<String, Function<Event, Mono<?>>> actionsMap;	
	private static final Logger logger = LoggerFactory.getLogger(KafkaPublisherSubscriberAdapter.class);
	private final KafkaSubscriberValidationHelper kafkaSubscriberValidationHelper;
	
	public KafkaPublisherSubscriberAdapter(@Qualifier("publisherProjectorAdapter")ProjectorPort projectorPort,
	...
	private Map<String, Function<Event, Mono<?>>> initializeActionsMap() {
		...
		return Map.of(GlobalConstants.PUBLI_CREATED, projectorPort::create, GlobalConstants.PUBLI_UPDATED,
				projectorPort::update, GlobalConstants.PUBLI_DELETED, projectorPort::deleteById);
	}

	@KafkaListener(topics = { GlobalConstants.PUBLI_CREATED, GlobalConstants.PUBLI_UPDATED, GlobalConstants.PUBLI_DELETED },
			groupId = "bookmgmt-consumer-publi")
	@Override
