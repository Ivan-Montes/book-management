...
@Component
public class KafkaBookBookshopSubscriberAdapter implements SubscriberPort {

	private final ProjectorPort projectorPort;
	private final Map<String, Function<Event, Mono<?>>> actionsMap;	
	private static final Logger logger = LoggerFactory.getLogger(KafkaBookBookshopSubscriberAdapter.class);
	private final KafkaSubscriberValidationHelper kafkaSubscriberValidationHelper;
	
	public KafkaBookBookshopSubscriberAdapter(@Qualifier("bookBookshopProjectorAdapter")ProjectorPort projectorPort,
	...
	private Map<String, Function<Event, Mono<?>>> initializeActionsMap() {
		...
		return Map.of(GlobalConstants.BBS_CREATED, projectorPort::create, GlobalConstants.BBS_UPDATED,
				projectorPort::update, GlobalConstants.BBS_DELETED, projectorPort::deleteById);
	}

	@KafkaListener(topics = { GlobalConstants.BBS_CREATED, GlobalConstants.BBS_UPDATED, GlobalConstants.BBS_DELETED },
			groupId = "bookmgmt-consumer-bbs")
	@Override
