...
public class PublisherReadRepositoryAdapter implements ReadRepositoryPort<Publisher>, RequestByNameReadRepositoryPort {
	...
	public PublisherReadRepositoryAdapter(SessionFactory sessionFactory, PublisherMapper mapper) {
		super();
	...
	@Override
	public Mono<Page<Publisher>> findAll(Pageable pageable) {

		CriteriaBuilder cb = sessionFactory.getCriteriaBuilder();
		CriteriaQuery<PublisherJpaEntity> cq = cb.createQuery(PublisherJpaEntity.class);
		Root<PublisherJpaEntity> root = cq.from(PublisherJpaEntity.class);
	    ...
	    Root<PublisherJpaEntity> countRoot = countQuery.from(PublisherJpaEntity.class);
	    ...
	    Mono<List<PublisherJpaEntity>> entityMono = Mono.from(dataQuery.convert().with(UniReactorConverters.toMono()));
	    Mono<Long> countMono = Mono.from(countUni.convert().with(UniReactorConverters.toMono()));

	    return Mono.zip(entityMono, countMono)
	        .map(tuple -> {
	            List<Publisher> domList = convertToDom(tuple.getT1());
	            long total = tuple.getT2();
	            return convertToPage(domList, pageable, total);
	        });
	}

	private List<Publisher> convertToDom(List<PublisherJpaEntity> list) {
	...
	private Page<Publisher> convertToPage(List<Publisher> list, Pageable pageable, long total) {
		
		return new PageImpl<>(list, pageable, total);
	}
	
	private List<Order> buildSortOrder(Sort sort, CriteriaBuilder criteriaBuilder, Root<PublisherJpaEntity> root) {
	...
	@Override
	public Mono<Publisher> findById(UUID id) {
		...
		String queryString = """
				...
				WHERE e.publisherId = ?1
				""";

		return sessionFactory
				.withSession(session -> session.createQuery(queryString, PublisherJpaEntity.class)
						.setParameter(1, id)
	...
	public Mono<Boolean> existsByName(String name) {
		...
		String jpql = "SELECT COUNT(b) FROM PublisherJpaEntity b WHERE b.name = :name";
		return sessionFactory
				.withSession(
	...
	public Mono<Boolean> findByNameAndIdNot(String name, UUID id) {
	    ...
	    Predicate conditionId = cb.notEqual(root.get(GlobalConstants.PUBLI_ID), id);
