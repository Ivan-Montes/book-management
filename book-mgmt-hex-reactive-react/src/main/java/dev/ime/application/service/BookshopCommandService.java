...
public class BookshopCommandService implements CommandServicePort<BookshopDto> {
	...
	@Override
	public Mono<Event> create(BookshopDto item) {

		return Mono.justOrEmpty(item)
				.switchIfEmpty(Mono.error(new ResourceNotFoundException(Map.of(GlobalConstants.BS_CAT, GlobalConstants.MSG_REQUIRED))))						
				.map(dto -> new CreateBookshopCommand(UUID.randomUUID(), dto.name()))
				.flatMap(this::runHandler)
				.flatMap(this::processEvents);	
	}

	@Override
	public Mono<Event> update(UUID id, BookshopDto item) {
		
		return Mono.justOrEmpty(item)
				.switchIfEmpty(Mono.error(new ResourceNotFoundException(Map.of(GlobalConstants.BS_CAT, GlobalConstants.MSG_REQUIRED))))						
				.map(dto -> new UpdateBookshopCommand(id, dto.name()))
				.flatMap(this::runHandler)
				.flatMap(this::processEvents);	
	}
